 ...
 * Teaser only — full 28-name wall lives on /clients (avoids duplicating
 * the whole list here, which made /clients pointless).
 */
export default function ClientWall() {
  const teaser = site.clients.slice(0, 12);
  ...
  return (
    <section className="mx-auto w-full max-w-site px-6 py-20 md:px-16 md:py-32">
      ...
      <Reveal delay={0.1} className="mt-10">
        <Link
          href="/clients"
          className="text-sm font-medium text-charcoal underline decoration-ember decoration-2 underline-offset-4 transition-colors hover:text-ember"
        >
          All {site.clients.length} clients →
        </Link>
      </Reveal>
    </section>
  );
